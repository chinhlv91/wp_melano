<?php
	...
	/*
	*
	*	Coo Page Builder - Products Function Class
	*	------------------------------------------------
	*	Cootheme
	* 	http://www.cootheme.com
	*	
	*	ct_mini_product_items()
	*	ct_product_items()
	*
	*/
	...
	/* MINI PRODUCTS
	================================================== */
	if (!function_exists('ct_mini_product_items')) {
		function ct_mini_product_items($asset_type, $category, $item_count, $sidebar_config, $width) {
			
			global $woocommerce, $ct_catalog_mode;
			...
			if ($asset_type == "latest-products") {
				...
				if ( empty( $viewed_products ) )
					return '<p class="no-products">'.__( "You haven't viewed any products yet.", "cootheme").'</p>';
	...
	/* STANDARD PRODUCTS
	================================================== */
	if (!function_exists('ct_product_items')) {
		function ct_product_items($asset_type, $category, $carousel, $product_size, $item_count, $width) {
		    ...
		    } else if ($ct_sidebar_config == "one-sidebar") {
		    	if ($width == "3/4") {
		    	    $woocommerce_loop['columns'] = apply_filters( 'loop_shop_columns', 3 );
		    	    $columns = 3;	   	    
		    	} else if ($width == "1/2") {
