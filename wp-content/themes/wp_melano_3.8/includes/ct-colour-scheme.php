<?php
	...
	*/
	...
	$ct_customize_item_array = array(
			...
			array ( 'ct_icon_color' => 'Icon Color' ),
	...
	function ct_get_current_color_scheme_id() {
			
		$ct_coo_options = get_option ( 'ct_coo_options' );
		
		if ( isset( $ct_coo_options['colour_scheme_select_scheme'] ) ) {
			return $ct_coo_options['colour_scheme_select_scheme'];
		} else {
			return 0;
		}
	...
	function ct_get_schema_info($schema_id) {
	
		global $ct_customize_item_array;
		
		if ( $schema_id ) {
		
			$schemas = get_option ( 'ct_coo_options_schemes_saved' );
	...
	function ct_get_color_scheme_list( $extra = true ) {
...
//			$dropdown_values = array('' => '') + $dropdown_values; 
	...
	function ct_add_schema($schema_array = array()) {
		...
		if (!empty($schema_array)) {
			...
			foreach ($schema_array as $schema ) {
				...
				if (strtolower($schema['Setting Name']) == "color scheme name") {
					...
					foreach ( $ct_customize_item_array as $ct_customize_id => $ct_customize_data ) {
											
						if ( $schema['Setting Name'] == key($ct_customize_data) && isset($schema['Setting Value']) ) {
							
							$newSchema[key($ct_customize_data)] = $schema['Setting Value'];
		...
		$schemas = get_option ( 'ct_coo_options_schemes_saved' ); // get list of schemas
		...
		update_option ( 'ct_coo_options_schemes_saved', $schemas );
					
		}
		
	} // ct_add_schema
	
	function ct_delete_schema_ajax() {
	...
	add_action("wp_ajax_ct_delete_schema_ajax", "ct_delete_schema_ajax");
	add_action("wp_ajax_nopriv_ct_delete_schema_ajax", "ct_delete_schema_ajax");
	
	
	function ct_add_schema_ajax() {
		...
		if ($file_id) {
			...
			require_once (CT_INCLUDES_PATH . '/ct-colour-scheme/File_CSV_DataSource/DataSource.php');
			
		    $time_start = microtime(true);
		    $csv = new File_CSV_DataSource;

		    ct_stripBOM($file_path);
	...
	add_action("wp_ajax_ct_add_schema_ajax", "ct_add_schema_ajax");
	add_action("wp_ajax_nopriv_ct_add_schema_ajax", "ct_add_schema_ajax");
	
	
	function ct_use_schema_ajax() {
		...
		if ( $schema_id ) {
			...
			$schema_info = ct_get_schema_info ( $schema_id );
			...
			if ( !empty($schema_info) ) {
				...
				foreach ($schema_info as $key => $value) {
					...
					// print_r($ct_customize_item_array);
					
					foreach ( $ct_customize_item_array as $item) {
...
//			print_r ($schema_info);
	...
	add_action("wp_ajax_ct_use_schema_ajax", "ct_use_schema_ajax");
	add_action("wp_ajax_nopriv_ct_use_schema_ajax", "ct_use_schema_ajax");
	
	
	function ct_save_schema_ajax() {
...
//			print_r ($schemas); exit;
	...
	add_action("wp_ajax_ct_save_schema_ajax", "ct_save_schema_ajax");
	add_action("wp_ajax_nopriv_ct_save_schema_ajax", "ct_save_schema_ajax");
	
	
	
    // delete BOM from UTF-8 file
    function ct_stripBOM($fname) {
	...
	function ct_get_schema_select_html() {

		global $ct_customize_item_array;
				
		$schemas_array = get_option ( 'ct_coo_options_schemes_saved' );
		
		$html = '<select name="ct_coo_options[colour_scheme_select_scheme]" id="colour_scheme_select_scheme"> ';
		
		$dropdown_values = ct_get_color_scheme_list();
	...
	} // ct_get_schema_select_html
	
	
	function ct_get_schema_html_ajax() {
		
		$schema_id = $_POST['schema_id'];
		
		$html = ct_get_current_color_scheme_html_preview ( $schema_id );
	...
	add_action("wp_ajax_ct_get_schema_html_ajax", "ct_get_schema_html_ajax");
	add_action("wp_ajax_nopriv_ct_get_schema_html_ajax", "ct_get_schema_html_ajax");
	
	
	function ct_is_current_color_settings_empty () {

		global $ct_customize_item_array;
		
		$values = 0;
		
		if (!empty($ct_customize_item_array)) {
			foreach ( $ct_customize_item_array as $setting ) {
	...
	function ct_get_current_color_scheme_html_preview( $schema_id = false ) {
...
//		echo "++++".$schema_id;
		...
		if ( !$schema_id ) {
			$schema_id = ct_get_current_color_scheme_id();
		}
				
		$schemas_array = get_option ( 'ct_coo_options_schemes_saved' );
			
		$html = "<ul id=\"ct_coo_options-color-list\" class=\"color-list\">";
		...
		if ( !$schema_id || $schema_id == "0" ) {
		    ...
		    if ( $values == 0) {
			    ...
			    $html .= '<li><strong>'.__('No saved values exist yet', 'coo-theme-admin').'</strong><br/>'.__('Go to the theme customizer to save your colour options.', 'coo-theme-admin').'</li>';
			    
		    }

	    } else {
	    
			//echo ct_get_current_color_scheme_id();
	...
	function ct_export_color_scheme_html() {
		
		global $ct_customize_item_array;
		
		$html = '<p><input type="text" maxlength="20" name="ct-export-scheme-name" id="ct-export-scheme-name" placeholder="Name The Schema" />';
		
		$export_link = ct_create_export_download_link('url');
		
		$html .= '<a class="button-primary" id="ct-export-scheme-dl" href="'.$export_link.'">Download CSV</a></p>';
		
		return $html;
		
	}
	
	function ct_download_file($content = null, $file_name = null) {
	
		global $ct_customize_item_array;
	
	    if (! wp_verify_nonce($_REQUEST['nonce'], 'ct_coo_options_schema_action') )
	...
	function ct_create_export_download_link($echo = false){
	    $site_url = home_url();
	    $args = array(
	        'ct_coo_options_schema_action' => 'export_schema',
	        'nonce' => wp_create_nonce('ct_coo_options_schema_action')
	    );
	...
	function ct_add_query_var_vars() {
	    global $wp;
	    $wp->add_query_var('ct_coo_options_schema_action');
	}
	
	//then add a template redirect which looks for that query var and if found calls the download function

	function ct_admin_redirect_download_files(){
	    global $wp;
	    global $wp_query;
	    //download theme export
	    if (array_key_exists('ct_coo_options_schema_action', $wp->query_vars) && $wp->query_vars['ct_coo_options_schema_action'] == 'export_schema') {
	        ct_download_file();
	        die();
	    } else if (array_key_exists('ct_coo_options_schema_action', $wp->query_vars) && $wp->query_vars['ct_coo_options_schema_action'] == 'import_scheme') {
	    	// db_import_scheme();
	    	die();
	    }
	}
	...
	add_action('template_redirect', 'ct_admin_redirect_download_files');
	add_filter('init', 'ct_add_query_var_vars');
