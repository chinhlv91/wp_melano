...
(function(){
	...
	var superSearch = {
		init: function() {
			...
			jQuery('.coo-search-link').on('click', function(e) {
				e.preventDefault();
			...
			jQuery('.super-search-go').on('click', function(e) {
				e.preventDefault();
				var parentSearch = jQuery(this).parents('.ct-super-search'),
	...
	var header = {
		...
		headerOverlaySet: function() {
			...
			if (jQuery('#main-container').find('#coo-slider').length === 0 && jQuery('#main-container').find('.home-slider-wrap').length === 0 && jQuery('#page-wrap').find('.page-heading').length === 0) {
				jQuery('.inner-page-wrap').animate({
					'padding-top': headerWrapHeight + 30
				}, 300);
			} else if (!(jQuery('#main-container').find('#coo-slider').length > 0 || jQuery('#main-container').find('.home-slider-wrap').length > 0)) {
			...
			if (jQuery('#main-container').find('#coo-slider').length > 0 || jQuery('#main-container').find('.home-slider-wrap').length > 0) {
				if (jQuery('.page-heading').hasClass('fancy-heading')) {
		...
		ajaxSearch: function(e) {			
			var searchInput = jQuery(e.currentTarget),
				searchValues = searchInput.parents('form').serialize() + '&action=ct_ajaxsearch',
    ...
    jQuery(document).ready(function () {
        jQuery('nav#mobile-menu ul.menu li ul').parent().prepend('<span class="caret"></span>');
        jQuery('nav#mobile-menu ul.menu li ul').find('ul').hide();

        jQuery('.caret').addClass('caret-close');

        jQuery('.caret').click(function() {
            if(jQuery(this).parent().find('ul.sub-menu').is(':visible')){
                jQuery(this).removeClass('caret-open').addClass('caret-close');
                jQuery(this).parent().find('ul.sub-menu').slideUp(300).removeClass('open');
            }else{
                jQuery(this).removeClass('caret-close').addClass('caret-open');
                if(jQuery(this).parent().parent().hasClass('menu')){
                    jQuery('nav#mobile-menu ul.menu li ul').hide();
                }
                jQuery(this).parent().find('ul.sub-menu').slideDown(300).addClass('open').find('ul').hide();
            }
        });
    });
		...
		variations: function() {
			jQuery('.variations select').each( function() {
				...
				variationSelect.live("change", function(){
					if (jQuery('#ct-included').hasClass('has-productzoom')) {
	...
	var flexSlider = {
		init: function() {
			...
			if (jQuery('#ct-included').hasClass('has-productzoom') && !body.hasClass('mobile-browser')) {
				hasProductZoom = true;
			}
			...
			jQuery('#coo-slider').flexslider({
				...
				start: function(postsSlider) {
					jQuery('.coo-slider-loading').fadeOut(200);
			...
			jQuery('#coo-slider li').hover(function() {
	...
	var carouselWidgets = {
		...
		carouselSwipeIndicator: function(carousel) {
			carousel.appear(function() {
				var swipeIndicator = jQuery(this).parents('.carousel-overflow').find('.ct-swipe-indicator');
				setTimeout(function() {
	...
	var widgets = {
		...
		introAnimations: function() {
			...
			if (!isMobileAlt) {
				jQuery('.ct-animation').each(function() {
					...
					animatedItem.appear(function() {				
						if (itemAnimation == 'fade-from-left') {
							...
							setTimeout(function(){ 
								animatedItem.addClass('ct-animate');
							}, itemDelay);
						} else {
							setTimeout(function() {
								animatedItem.addClass('ct-animate');
		...
		countAssets: function() {
			jQuery('.ct-count-asset').each(function() {
		...
		countdownAssets: function() {
			jQuery('.ct-countdown').each(function() {
		...
		imageBanners: function() {
			jQuery('.ct-image-banner').each(function() {
	...
	var parallax = {
		init: function() {
			...
			jQuery('.spb_parallax_asset').each(function() {
				...
				if (parallaxAsset.hasClass('ct-parallax-video')) {
	...
	var onReady = {
		init: function(){
			page.init();
			if (jQuery('.ct-super-search').length > 0) {
			superSearch.init();
			}
			header.init();
			nav.init();
