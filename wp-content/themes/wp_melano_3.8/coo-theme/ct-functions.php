<?php
	...
	*	ct_list_galleries()
	*	ct_portfolio_related_posts()
	*	ct_has_previous_posts()
	*	ct_has_next_posts()
	*	ct_admin_altbg_css()
	*
	*/
	...
	/* SHORTCODE FIX
	================================================== */	 
	if (!function_exists('ct_content_filter')) {
		function ct_content_filter($content) {
			// array of custom shortcodes requiring the fix 
			$block = join("|",array("alert","ct_button","icon","ct_iconbox","ct_imagebanner","social","ct_social_share","highlight","decorative_ampersand","blockquote1","blockquote2","blockquote3","pullquote","dropcap1","dropcap2","dropcap3","dropcap4","one_half","one_half_last","one_third","one_third_last","two_third","two_third_last","one_fourth","one_fourth_last","three_fourth","three_fourth_last","one_half","one_half_last","progress_bar","chart","ct_count","ct_countdown","ct_tooltip","ct_modal","ct_fullscreenvideo","ct_visibility","table","trow","thcol","tcol","pricing_table","pt_column","pt_package","pt_details","pt_button","pt_price","labelled_pricing_table","lpt_label_column","lpt_row_label","lpt_column","lpt_price","lpt_package","lpt_row_label","lpt_row","lpt_button","list","list_item","hr","accordion","panel","tabs","tab","ct_supersearch","gallery","spb_accordion","spb_accordion_tab","blog","boxed_content","clients","clients_featured","codesnippet","divider","faqs","ct_gallery","googlechart","spb_gmaps","impact_text","jobs","jobs_overview","latest_tweets","spb_message","spb_parallax","portfolio","portfolio_carousel","portfolio_showcase","posts_carousel","spb_products","spb_products_mini","spb_raw_html","spb_raw_js","recent_posts","spb_slider","sitemap","search_widget","supersearch","spb_tabs","spb_tab","spb_text_block","team_carousel","team","testimonial","testimonial_carousel","testimonial_slider","fullwidth_text","spb_toggle","spb_tour","tweets_slider","spb_video","blank_spacer","spb_single_image"));
			// opening tag
		...
		add_filter("the_content", "ct_content_filter");
	}
	
	
	/* TWEET FUNCTIONS
	================================================== */
	if (!function_exists('ct_get_tweets')) {
		function ct_get_tweets($twitterID, $count) {
						...
						foreach($tweets as $tweet) {
						    ...
						    if ($tweet['text']) {
						        ...
						        */
						        ...
						        $dago = human_time_diff($date, $blogtime) . ' ' . sprintf(__('ago', 'cootheme')); // calculates and outputs the time past in human readable format
	...
	function ct_twitter_users($text) {
	       $text = preg_replace('/([\.|\,|\:|\>|\{|\(]?)@{1}(\w*)([\.|\,|\:|\!|\?|\>|\}|\)]?)\s/i', "$1<a href=\"http://twitter.com/$2\" class=\"twitter-user\">@$2</a>$3 ", $text);
	       return $text;
	}

    function ct_encode_tweet($text) {
	...
	================================================== */
	if (!function_exists('ct_latest_tweet')) {
		function ct_latest_tweet($count, $twitterID) {
		
			global $ct_include_twitter;
			$ct_include_twitter = true;
			
			$content = "";
			
			if ($twitterID == "") {
				return __("Please provide your Twitter username", "cootheme");
			}
	...
	================================================== */	
	function ct_portfolio_related_posts( $post_id ) {
	    $query = new WP_Query();
	...
	function ct_has_previous_posts() {
		ob_start();
		previous_posts_link();
	...
	/* ADMIN ALT BG CSS
	================================================== */
	if (!function_exists('ct_admin_altbg_css')) {
		function ct_admin_altbg_css() {
			// Alt Background
		...
		<?php }
		add_action( 'admin_head', 'ct_admin_altbg_css' );
	}
?>