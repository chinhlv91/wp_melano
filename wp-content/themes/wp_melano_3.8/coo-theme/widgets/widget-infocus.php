<?php
	...
	*/
	...
	// Register widget
	add_action( 'widgets_init', 'init_ct_infocus' );
	function init_ct_infocus() { return register_widget('ct_infocus'); }
	
	class ct_infocus extends WP_Widget {
		function ct_infocus() {
			parent::WP_Widget( 'ct_infocus_widget', $name = 'Cootheme In Focus' );
		}
		...
		function widget( $args, $instance ) {
			...
			<div class="infocus-item">
				...
				<?php 
					...
					$thumb_image = rwmb_meta('ct_thumbnail_image', 'type=image&size=full', $post_id);
					$thumb_video = get_post_meta($post_id, 'ct_thumbnail_video_url', true);
				...
				<figure class="animated-overlay overlay-alt">
					<?php if ($thumb_video != "") { ?>
						<?php echo ct_video_embed($thumb_video, 300, 200); ?>
					<?php } else if ($image) { ?>
		...
		function form( $instance ) {	
				...
				<p>
					<label for="<?php echo $this->get_field_id('post_id'); ?>"><?php echo __( 'Post ID:', 'coo-theme-admin' ); ?></label>
