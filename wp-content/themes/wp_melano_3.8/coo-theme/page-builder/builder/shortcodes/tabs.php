<?php
	...
	/*
	*
	*	Coo Page Builder - Tabs Shortcode
	*	------------------------------------------------
	*	Cootheme
	...
	*/
	...
	class CooPageBuilderShortcode_spb_tab extends CooPageBuilderShortcode {
	    public function content( $atts, $content = null ) {
	        $title = '';
	        extract(shortcode_atts(array(
	            'title' => __("Tab", "coo-page-builder")
	        ), $atts));
	        $output = '';
	    ...
	    public function contentAdmin($atts, $content) {
	        $title = '';
	        $defaults = array( 'title' => __('Tab', "coo-page-builder") );
	        extract( shortcode_atts( $defaults, $atts ) );
	
	        return '<div id="tab-'. sanitize_title( $title ) .'" class="row-fluid spb_column_container spb_sortable_container not-column-inherit">'. do_shortcode($content) . CooPageBuilder::getInstance()->getLayout()->getContainerHelper() . '</div>';
	    }
	}
	
	class CooPageBuilderShortcode_spb_tabs extends CooPageBuilderShortcode {
	
	    public function __construct($settings) {
	        parent::__construct($settings);
	        CooPageBuilder::getInstance()->addShortCode( array( 'base' => 'spb_tab' ) );
	    }
	        ...
	        foreach ( $this->settings['params'] as $param ) {
	            if ( $param['param_name'] != 'content' ) {
	                ...
	                if ( is_string($param['value']) ) {
	                    $shortcode_attributes[$param['param_name']] = __($param['value'], "coo-page-builder");
	                } else {
	                    $shortcode_attributes[$param['param_name']] = $param['value'];
	                }
	    ...
	    public function content($atts, $content =null) {
	        ...
	        if ( 'spb_tour' == $this->shortcode) {
	            $output .= "\n\t\t\t" . '<div class="spb_tour_next_prev_nav"> <span class="spb_prev_slide"><a href="#prev" title="'.__('Previous slide', "cootheme").'">'.__('Previous slide', "cootheme").'</a></span> <span class="spb_next_slide"><a href="#next" title="'.__('Next slide', "cootheme").'">'.__('Next slide', "cootheme").'</a></span></div>';
	        }
	...
	SPBMap::map( 'spb_tabs', array(
	    ...
	    "params"	=> array(
	        array(
	            ...
	            "value" => "",
	            "description" => __("What text use as widget title. Leave blank if no title is needed.", "coo-page-builder")
	        ),
	        array(
	            "type" => "textfield",
		...
		<div class="tab_controls">
			<button class="add_tab">'.__("Add New Tab", "coo-page-builder").'</button>
	</div>
	    ...
	    'default_content' => '
		<ul>
			<li><a href="#tab-1"><span>'.__('Tab 1', 'coo-theme-admin').'</span></a><a class="edit_tab"></a><a class="delete_tab"></a></li>
			<li><a href="#tab-2"><span>'.__('Tab 2', 'coo-theme-admin').'</span></a><a class="edit_tab"></a><a class="delete_tab"></a></li>
		</ul>
	
		<div id="tab-1" class="row-fluid spb_column_container spb_sortable_container not-column-inherit">
			[spb_text_block width="1/1"] '.__('This is a text block. Click the edit button to change this text.', 'coo-theme-admin').' [/spb_text_block]
		</div>
