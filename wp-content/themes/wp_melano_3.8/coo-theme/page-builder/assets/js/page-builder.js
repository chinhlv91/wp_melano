...
//            });
                ...
                if (cur_el.hasClass('spb_column') || cur_el.hasClass('spb_tabs') || cur_el.hasClass('spb_tour') || cur_el.hasClass('spb_accordion')) {
                    ...
                    if (cur_el.find('.spb_element_wrapper .spb_column_container').length > 0) {
                        ...
                        cur_el.find('.spb_element_wrapper .spb_column_container').each(function (index) {
                            $.coo_page_builder.addLastClass($(this)); // Seems it does nothing
        ...
        }, // endjQuery.coo_page_builder.addLastClass()
...
jQuery(document).ready(function($) {
	...
	$('.spb_main_sortable').sortable({
		...
		update: function() {$.coo_page_builder.save_spb_html(); }
	});
    */
    
    var currentAsset = "";
    
    $( "#coo_page_builder .dropable_el, #coo_page_builder .dropable_column" ).draggable({
	...
	if ( $("#coo_page_builder").length == 1 ) {
		$('div#postdivrich').before('<p class="page-builder-switch"><a class="spb_switch-to-builder button-primary" href="#">Coo Page Builder</a></p>');

		var postdivrich = $('#postdivrich'),
			cooPageBuilder = $('#coo_page_builder');
		...
		if ( $('#spb_js_status').val() == 'true' && jQuery('#wp-content-wrap').hasClass('tmce-active') ) {
			...
			postdivrich.show();
			cooPageBuilder.hide();
			$('.spb_switch-to-builder').html('Coo Page Builder');
		}
	}
    ...
    $.coo_page_builder.galleryImagesControls(); /* Actions for gallery images handling */
    ...
    $('#coo_page_builder').on('click', '.add-text-block-to-content', function(e) {
        e.preventDefault();
...
function spb_templateSystem() {
	...
	jQuery('.ct_prebuilt_template a').live("click", function(e) {
		e.preventDefault();
		
		var data = {
			action: 'ct_load_template',
...
function spb_shortcodesToBuilder() {
	...
	jQuery.post(ajaxurl, data, function(response) {
        ...
        jQuery.coo_page_builder.isMainContainerEmpty();
		//
		//console.log(response);
		jQuery.coo_page_builder.addLastClass(jQuery(".spb_main_sortable"));
		initDroppable();
...
function initDroppable() {
    jQuery('.spb_sortable_container').sortable({
        ...
        update: function() {jQuery.coo_page_builder.save_spb_html(); },
...
function showEditForm(element) {
	...
	jQuery.post(ajaxurl, data, function(response) {
        ...
        jQuery.coo_page_builder.initializeFormEditing(element);
...
function addLastClass(dom_tree) {
    return jQuery.coo_page_builder.addLastClass(dom_tree);
