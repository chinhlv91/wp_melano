<?php
	...
	*/
	...
	function ct_product_items_text($count) {
			
		$product_item_text = "";
		
    	if ( $count > 1 ) {
        	$product_item_text = str_replace('%', number_format_i18n($count), __('% items', 'cootheme'));
        } elseif ( $count == 0 ) {
        	$product_item_text = __('0 items', 'cootheme');
        } else {
        	$product_item_text = __('1 item', 'cootheme');
        }
	...
	================================================== */ 
	if (!function_exists('ct_woo_header_add_to_cart_fragment')) {
		function ct_woo_header_add_to_cart_fragment( $fragments ) {
			global $woocommerce;
			...
			<li class="parent shopping-bag-item">
				<a class="cart-contents" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('View your shopping cart', 'cootheme'); ?>"><i class="ss-cart"></i><?php echo $woocommerce->cart->get_cart_total(); ?></a>
					...
					<li>                                      
						<div class="shopping-bag">
							...
							<?php if ( $cart_count != "0" ) { ?>
								...
								<div class="bag-header"><?php echo $cart_count_text; ?> <?php _e('in the shopping bag', 'cootheme'); ?></div>
								        ...
								        <?php if ($bag_product->exists() && $cart_item['quantity']>0) { ?>                                            
								        	...
								        	<div class="bag-product clearfix">   	
									            ...
									            <div class="bag-product-details">
									            	...
									            	<div class="bag-product-price"><?php _e("Unit Price:", "cootheme"); ?> <?php echo woocommerce_price($bag_product->get_price()); ?></div>
									            	<div class="bag-product-quantity"><?php _e('Quantity:', 'cootheme'); ?> <?php echo $cart_item['quantity']; ?></div>
									            </div>
							    ...
							    <div class="bag-buttons">
							    	...
							    	<a class="ct-button standard ct-icon-reveal bag-button" href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>"><i class="ss-view"></i><span class="text"><?php _e('View shopping bag', 'cootheme'); ?></span></a>
							    
							    	<a class="ct-button standard ct-icon-reveal checkout-button" href="<?php echo esc_url( $woocommerce->cart->get_checkout_url() ); ?>"><i class="ss-creditcard"></i><span class="text"><?php _e('Proceed to checkout', 'cootheme'); ?></span></a>
							                    
								</div>
				
							<?php } else { ?>
									
								<div class="bag-header"><?php _e("0 items in the shopping bag", "cootheme"); ?></div>
									
								<div class="bag-empty"><?php _e('Unfortunately, your shopping bag is empty.','cootheme'); ?></div>
														
								<div class="bag-buttons">
									
									<?php $shop_page_url = get_permalink( woocommerce_get_page_id( 'shop' ) ); ?>
									
									<a class="ct-button standard ct-icon-stroke checkout-button" href="<?php echo esc_url( $shop_page_url ); ?>"><i class="ss-cart"></i><span class="text"><?php _e('Shopping Now', 'cootheme'); ?></span></a>
		...
		add_filter('add_to_cart_fragments', 'ct_woo_header_add_to_cart_fragment');
	}
		...
		function ct_wishlist_button() {
			...
			if ( class_exists( 'YITH_WCWL_UI' ) )  {
				...
				$html = '<div class="yith-wcwl-add-to-wishlist">'; 
				    ...
				    $html .= '><a href="' . htmlspecialchars($yith_wcwl->get_addtowishlist_url()) . '" data-product-id="' . $product->id . '" data-product-type="' . $product_type . '" ' . $classes . ' ><i class="fa-heart"></i></a>';
				    $html .= '</div>';
				
				$html .= '<div class="yith-wcwl-wishlistaddedbrowse hide" style="display:none;"><span class="feedback">' . __( 'Product added to wishlist.', 'cootheme' ) . '</span> <a href="' . $url . '"><i class="fa-check"></i></a></div>';
	...
	add_action( 'woocommerce_single_product_summary', 'ct_product_accordion', 35);
	}
	add_action( 'woocommerce_single_product_summary', 'ct_product_share', 45);
	
	
	/* PRODUCT ACCORDION
	================================================== */ 
	if (!function_exists('ct_product_accordion')) {
		function ct_product_accordion() {
				...
				<div class="panel">
					<div class="panel-heading">
						<a class="accordion-toggle collapsed" data-toggle="collapse" data-parent="#product-accordion" href="#additional-information">
							<?php _e("Additional Information", "cootheme"); ?>
						</a>
					</div>
				...
				<div class="panel">
					<div class="panel-heading">
						<a class="accordion-toggle collapsed" data-toggle="collapse" data-parent="#product-accordion" href="#reviews">
							<?php _e("Reviews", "cootheme"); ?> (<?php echo comments_number( '0', '1', '%' ); ?>)
						</a>
					</div>
	...
	/* WOO PRODUCT SHARE
	================================================== */ 
	if (!function_exists('ct_product_share')) {
		function ct_product_share() {
			global $post;
			$src = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), false, '' );
		?>
			<div class="product-share clearfix">
				<span><?php _e("Share", "cootheme"); ?></span>
	...
	/* WOO HELP BAR
	================================================== */ 
	if (!function_exists('ct_woo_help_bar')) {
		function ct_woo_help_bar() {
			$options = get_option('ct_coo_options');
		?>
			<div class="help-bar clearfix">
				<span><?php echo do_shortcode($options['help_bar_text']); ?></span>
				<ul>
				    <li><a href="#email-form" class="inline" data-toggle="modal"><?php _e("Email customer care", "cootheme"); ?></a></li>
				    <li><a href="#shipping-information" class="inline" data-toggle="modal"><?php _e("Shipping information", "cootheme"); ?></a></li>
				    <li><a href="#returns-exchange" class="inline" data-toggle="modal"><?php _e("Returns & exchange", "cootheme"); ?></a></li>
				    <li><a href="#faqs" class="inline" data-toggle="modal"><?php _e("F.A.Q.'s", "cootheme"); ?></a></li>
				</ul>
			</div>
				...
				<div class="modal-dialog">
				    <div class="modal-content">
						<div class="modal-header">
							...
							<h3 id="email-form-modal"><?php _e("Email customer care", "cootheme"); ?></h3>
						</div>
			...
			<div id="shipping-information" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="shipping-modal" aria-hidden="true">
				<div class="modal-dialog">
				    <div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="ss-delete"></i></button>
							<h3 id="shipping-modal"><?php _e("Shipping information", "cootheme"); ?></h3>
						</div>
						<div class="modal-body">	
			...
			<div id="returns-exchange" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="returns-modal" aria-hidden="true">
				<div class="modal-dialog">
				    <div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="ss-delete"></i></button>
							<h3 id="returns-modal"><?php _e("Returns & exchange", "cootheme"); ?></h3>
						</div>
						<div class="modal-body">					
			...
			<div id="faqs" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="faqs-modal" aria-hidden="true">
				<div class="modal-dialog">
				    <div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="ss-delete"></i></button>
							<h3 id="faqs-modal"><?php _e("F.A.Q.'s", "cootheme"); ?></h3>
						</div>
						<div class="modal-body">
